-- =====================================================================
-- 👑【小木HUB - 现代化侧边栏高级版 v3.5 (完整功能+玩家管理)】
-- =====================================================================
...
local VehicleModule = { Enabled = false, Speed = 100 }
...
createToggleOption(movePanel, 0, "速度加快", SpeedModule.Enabled, function(state)
    SpeedModule.Enabled = state
end)
createSliderOption(movePanel, 48, "移动速度", 16, 200, SpeedModule.CustomSpeed, "速", function(val)
    SpeedModule.CustomSpeed = val
end)
createToggleOption(movePanel, 96, "跳跃加强", JumpModule.Enabled, function(state)
    JumpModule.Enabled = state
end)
createSliderOption(movePanel, 144, "跳跃高度", 50, 300, JumpModule.CustomJump, "高", function(val)
    JumpModule.CustomJump = val
end)
createToggleOption(movePanel, 192, "穿墙 (Noclip)", MiscModule.Noclip, function(state)
    MiscModule.Noclip = state
    noclipApplied = false
end)
createToggleOption(movePanel, 240, "人物飞行", FlightModule.Enabled, function(state)
    FlightModule.Enabled = state
    if not state then cleanupState() end
end)

local combatPanel = createContentPanel("战斗")
createToggleOption(combatPanel, 0, "超维自瞄", AimbotModule.Enabled, function(state) AimbotModule.Enabled = state end)
...
local function refreshPlayerList()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= plr then
            ...
            local isWhitelisted = AimbotModule.PlayerWhitelist[player.UserId]
            local isBlacklisted = AimbotModule.PlayerBlacklist[player.UserId]
...
local function switchTab(tabName)
    ...
    for name, panel in pairs(tabContentFrames) do
...
print("👑【小木HUB v3.5 完整版】加载成功！含玩家管理模块。")
